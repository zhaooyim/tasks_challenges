...
// For testing these routes:
// (especially the ones that need the checkJwt middleware function),
// we would use supertest for the route,
// but also vi.mock to make a mocked version of our checkJwt.

//  What would happen in industry?
// If this was a bigger app, to test the checkJwt function properly,
// you would build a 'fake'  `identity server`, aka a fake auth0,
// that would allow you to test the checkJwt function properly.
// This faked `identity server` would also be very useful for devs working on
// the front end, who would need to 'fake' a server response from an auth protected route.
